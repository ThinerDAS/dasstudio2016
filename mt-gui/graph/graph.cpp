                        ...
                        for ( int j : gbi ) for ( bools::iterator ik = gbi.lower_bound ( j + 1 ); ik != gbi.end(); ++ik )
                            {
                                ...
                                min_dist[j] = 0;
                                while ( !active_vs.empty() && !sc_smallflag )
                                {
                                ...
                                {
                                    sc_flag = false;
                                    eprintf ( "%d, %d nodes witnessed the importance of node %d\n", j, k, i );
                                    goto LOOP_END;
                                }
                            }
LOOP_END:
    ...
    category_mark& cm = m_cm;
    {
        ...
        for ( int i = 0; i < s; i++ )
        {
            ...
            else
            {
                ...
                else if ( m_vertices[i][EV_INTERNAL_FLAG] != 0 )
                {
                    door_vs.push_back ( i );
                }
                else
